...
* [{doc}`/reference/tokenizers/tokenngram`] Added new option `ignore_blank`.

  We can replace `TokenBigramIgnoreBlank` with `TokenNgram("ignore_blank", true)` as below.

  e.g) If we use `TokenBigram`.
  ...
  e.g) If we use `TokenBigramIgnoreBlank`.
  ...
  e.g) If we use `TokenNgram("ignore_blank", true)`.
